...
test("DSH details tools project the current OPL project and a contained read-only workspace", () => {
  ...
  assert.match(app, /tab === "opl-project-progress-panel"[\s\S]*projectProgress\.emptyReason === "projection_unavailable"[\s\S]*stateStatus !== "loading"/);
...
test("Settings uses the App-owned navigation groups and one shared read model", () => {
  ...
  assert.match(slotHost, /renderSettings\(selected, renderContribution \?\? undefined, setSelected, close\)/);

...
test("loopback HTTP host exposes standard thread lifecycle, subagent projection, SSE, and OPL passthrough", async (t) => {
  ...
  const canonicalReads = frames.slice(chatThreadStartIndex).filter((frame) => frame.method === "thread/read");
  assert.deepEqual(canonicalReads.map((frame) => ({
    direction: frame.direction,
    threadId: frame.params.threadId,
    includeTurns: frame.params.includeTurns
  })), [
    { direction: "client_to_server", threadId: chat.body.threadId, includeTurns: false },
    { direction: "client_to_server", threadId: chat.body.threadId, includeTurns: true }
  ]);
});

...
export function FeatureStatusPanel({ features, locale, destination, onNavigate, onRefresh, onAction, busy }: {
  ...
  return <section className="feature-status-panel" data-testid="opl-feature-status-panel" aria-label={zh ? '功能状态' : 'Feature status'}>
    <header><h2>{zh ? '功能状态' : 'Feature status'}</h2><button type="button" onClick={onRefresh}>{zh ? '刷新状态' : 'Refresh status'}</button></header>
    ...
    {rows.map(feature => <details key={feature.featureId} data-testid="opl-feature-status-row" data-feature-id={feature.featureId} data-state={feature.state}>
